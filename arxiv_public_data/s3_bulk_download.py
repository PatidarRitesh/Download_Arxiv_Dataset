...
# If you just want to download the PDFs 
...
def download_file(filename, outfile, chunk_size=CHUNK_SIZE, redownload=False,
    ...
    # ### Update mclupr : if the last downloaded file is corrupted, redownload it --------------#
    #--------------------------------------------------------------------------------------------
    if os.path.exists(outfile) and not redownload:
        ...
        try:
            ...
            logger.warning("{} Corrupted file {}, redownloading...".format(e, outfile))
            return download_file(filename, outfile, redownload=True)
    #------------------------------- Original Code -----------------------------------------------
    # if os.path.exists(outfile) and not redownload:
    #     md5 = hashlib.md5()
    #     md5.update(gzip.open(outfile, 'rb').read())
    ...
    if not dryrun:
        ...
        with gzip.open(outfile, 'wb') as fout:
            ...
            ############### update mclupr - added tqdm over downloading 1 file ##################
            #-------------------------------------------------------------------------------------
            with tqdm(total=int(request.headers['Content-Length'])) as pbar:
...
def get_manifest(filename=None, redownload=False):
    ...
    md5 = download_file(
        ...
        # S3_TEX_MANIFEST, manifest_file, redownload=redownload, dryrun=False
    )
...
def download_check_tarfile(filename, md5_expected, dryrun=False, redownload=False):
    ...
    if not dryrun:
        if md5_expected != md5_downloaded:
            ...
            ##################### update mclupr -  #####################------------------------
            # add warning to the logger that the md5sums do not match and redownloading the file
            # if the md5sums still do not match, raise an AssertionError
            #-----------------------------------------------------------------------------------
            logger.warning("MD5 '{}' does not match expected '{}' for file '{}'".format(
                md5_downloaded, md5_expected, filename
            ))
            logger.warning("Redownloading the file '{}'".format(filename))
            md5_downloaded = download_file(
                filename, outname, dryrun=dryrun, redownload=True
            )
...
def download_check_tarfiles(list_of_fileinfo, dryrun=False):
    ...
    """
    #____________________________________________________________________________________________
    # -------------------- update mclupr : added tqdm ------------------------------------------#
    for fileinfo in tqdm(list_of_fileinfo):
...
def process_tarfile_inner(filename, pdfnames=None, processes=1, dryrun=False,
    ...
    for tf in txtfiles:
        ...
        if not os.path.exists(dirname):
            call('mkdir -p {}'.format(dirname), dryrun)

        if not dryrun:
            shutil.move(tf, mvfn)

    # clean up pdfs
    call('rm -rf {}'.format(os.path.join(DIR_PDFTARS, basename)), dryrun)
